...
const getAvailableRooms = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Available rooms fetched successfully", rooms });
  } catch (error) {
    logger.error("Failed to fetch available rooms", error);
    return res.status(error.statusCode || 500).json({ message: error.message || "Error fetching available rooms" });
  }
};
...
const bookRoom = async (req, res) => {
  try {
    ...
    if (!userId) return res.status(401).json({ message: "Unauthorized: user not identified" });
    const { roomId, startDate, endDate } = req.body;
    if (!roomId || !startDate || !endDate) return res.status(400).json({ message: "roomId, startDate and endDate are required" });
    ...
    return res.status(201).json({ message: "Your booking request is created successfully and is pending approval.", booking });
  } catch (error) {
    logger.error("Failed to create booking", error);
    return res.status(error.statusCode || 500).json({ message: error.message || "Error booking room" });
  }
};
...
const getBookings = async (req, res) => {
  try {
    ...
    const canViewAll = bookingActions.delete === true || userActions.view === true;
    const canReviewMemberBookings = approvalActions.view === true;
    let bookings;
    if (canViewAll) bookings = await getAllBookings();
    else if (canReviewMemberBookings) bookings = await getMemberBookings();
...
const updateBookingHandler = async (req, res) => {
  try {
    ...
    if (!isOwner && bookingActions.update !== true) return res.status(403).json({ message: "Forbidden: You cannot update another user's booking" });
...
const deleteBookingHandler = async (req, res) => {
  try {
    ...
    const bookingActions = getPermission(userPermissions, "bookings");
    const isOwner = booking.user.toString() === userId.toString();
    if (!isOwner && bookingActions.delete !== true) return res.status(403).json({ message: "Forbidden: You cannot delete another user's booking" });
...
module.exports = { getAvailableRooms, bookRoom, getBookings, updateBookingHandler, deleteBookingHandler };
