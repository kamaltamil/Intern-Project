...
describe('recaptchaService', () => {
  ...
  test('returns false for unsuccessful verification', async () => { const request={on:jest.fn(),write:jest.fn(),end:jest.fn()}; https.request.mockImplementation((u,o,cb)=>{cb({on:jest.fn((e,fn)=>{if(e==='data')fn('{"success":false}');if(e==='end')fn();})});return request;}); await expect(verifyRecaptcha('token')).resolves.toBe(false); });
