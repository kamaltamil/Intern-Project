...
const registerUser = async ({ name, email, username, password, role }) => {
  ...
  if (!nameRegex.test(normalizedName)) {
    const error = new Error("Name must contain at least two words with at least 2 letters each");
    error.statusCode = 400;
    throw error;
  }
  ...
  if (!userNameRegex.test(normalizedUsername)) {
    const error = new Error("Username must be 3-16 characters and contain only letters, numbers, or underscores");
    error.statusCode = 400;
    throw error;
  }
  if (!passwordRegex.test(String(password))) {
    const error = new Error("Password must be at least 8 characters and include uppercase, lowercase, a number, and a special character");
    error.statusCode = 400;
    throw error;
  }
...
const loginUser = async (identifier, password) => {
  ...
  const roleDoc = user.role?.name ? user.role : await getRoleDocument(user.role);
...
const refreshAccessToken = async (refreshToken) => {
  ...
  return { token: newToken, refreshToken: newRefreshToken, role: roleName, permissions };
};
...
const validateProfileName = (value) => {
  ...
  if (!nameRegex.test(name)) {
    throwValidationError("Name must contain at least two words with at least 2 letters each");
  }
  return name;
};
...
const validateProfileUsername = async (value, userId) => {
  ...
  if (!userNameRegex.test(username)) {
    throwValidationError("Username must be 3-16 characters and contain only letters, numbers, or underscores");
  }
...
const validateAndHashProfilePassword = async (value) => {
  ...
  if (!passwordRegex.test(password)) {
    throwValidationError("Password must be at least 8 characters and include uppercase, lowercase, a number, and a special character");
  }
...
const updateOwnProfile = async (userId, payload) => {
  ...
  if (payload.email !== undefined) user.email = await validateProfileEmail(payload.email, userId);
  if (payload.username !== undefined) user.username = await validateProfileUsername(payload.username, userId);
  if (payload.password !== undefined) user.password = await validateAndHashProfilePassword(payload.password);
