...
const getReports = async () => {
  ...
  for (const booking of bookings) {
    ...
    if (booking.room?._id) {
      ...
      current.bookings += 1;
      if (ACTIVE_STATUSES.includes(booking.bookingStatus)) current.nights += nights;
  ...
  return {
    summary: {
      ...
      activeBookings: bookings.filter((b) => ACTIVE_STATUSES.includes(b.bookingStatus)).length,
