...
const getAllUsers = async (requestingUser = null) => {
  if (!requestingUser) {
    return await User.find().populate("role").select("-password -refreshToken").sort({ createdAt: -1 });
  }
...
const getUserById = async (id) =>
  await User.findById(id).populate("role").select("-password -refreshToken");
...
const validateCreateUserInput = ({ name, email, username, password, role }) => {
  if (!name || !email || !username || !password || !role) {
    throwBadRequest("Name, email, username, password and role are required");
  }
  const normalizedName = String(name).trim();
  const normalizedEmail = String(email).trim().toLowerCase();
  const normalizedUsername = String(username).trim().toLowerCase();
  if (!nameRegex.test(normalizedName)) {
    throwBadRequest("Name must contain at least two words with at least 2 letters each");
  }
  if (!emailRegex.test(normalizedEmail)) throwBadRequest("Please enter a valid email address");
  if (!userNameRegex.test(normalizedUsername)) {
    throwBadRequest("Username must be 3-16 characters and contain only letters, numbers, or underscores");
  }
  if (!passwordRegex.test(String(password))) {
    throwBadRequest("Password must be at least 8 characters and include uppercase, lowercase, a number, and a special character");
  }
...
const validateCreateRolePermission = async (requestingUser, roleDocument) => {
  ...
  if (!isAllowed) {
    throwForbidden("Forbidden: You are not authorized to create a user with this role");
  }
};
...
const createUser = async ({
  ...
  const existing = await User.findOne({
    $or: [{ email: normalized.normalizedEmail }, { username: normalized.normalizedUsername }],
  });
...
const validateManageableRole = async (user, payload, requestingUser) => {
  ...
  if (!allowedRoles.has(currentRoleStr)) {
    throwForbidden("Forbidden: You are not authorized to manage this user");
  }

  if (payload.role !== undefined && payload.role !== "") {
    const newRoleDoc = await findRoleByReference(payload.role);
    if (!allowedRoles.has(newRoleDoc._id.toString())) {
      throwForbidden("Forbidden: You cannot assign this role to a user");
    }
...
const validateDeletePermission = async (user, requestingUser) => {
  ...
  if (!allowedRoles.includes(currentRoleStr)) {
    throwForbidden("Forbidden: You are not authorized to delete this user");
  }
};
...
const deleteUser = async (id, currentUserId = null, requestingUser = null) => {
  if (currentUserId && currentUserId.toString() === id.toString()) {
    throwBadRequest("You cannot delete your own account");
  }
