...
const getFallbackRoleColor = (roleName) => {
  const match = ROLE_COLORS.find((c) => c.label.toLowerCase() === roleName?.toLowerCase());
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <Card className="rounded-2xl border border-[#ECE6DF] shadow-sm">
        <div className="flex items-center gap-4 mb-6">
          <Avatar size={72} src={previewImage} style={{ backgroundColor: '#C76A34', fontSize: 28 }}>
          ...
          <div>
            ...
            <Tag color={roleColor} className="mt-1">{roleName}</Tag>
          </div>
        </div>
        ...
        {isUserLoading ? (
          <div className="py-10"><p>Loading profile...</p></div>
        ) : isUserError ? (
          <div className="py-10"><p>Unable to load profile data.</p></div>
        ) : !editing ? (
          <div className="space-y-4">
            <Descriptions
              ...
              bordered
              items={[
                { key: '1', label: 'Full Name', children: <Text strong>{profileUser?.user?.name || '-'}</Text> },
                { key: '2', label: 'Email', children: <Text strong>{profileUser?.user?.email || '-'}</Text> },
                { key: '3', label: 'Username', children: <Text strong>@{profileUser?.user?.username || '-'}</Text> },
                { key: '4', label: 'Role', children: <Tag color={roleColor}>{roleName}</Tag> },
              ]}
            />
          ...
          <Form form={form} layout="vertical" onFinish={onFinish}>
            <Form.Item label="Profile Photo">
              ...
              >
                <Button icon={<UploadOutlined />} type="default">Choose Photo</Button>
              </Upload>
            </Form.Item>
