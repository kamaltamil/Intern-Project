...
function ApprovalPage() {
  ...
  const mutation = useMutation({
    mutationFn: ({ id, action }) =>
      action === "approve" ? approveBooking(id) : rejectBooking(id),
    onMutate: ({ id }) => setBusyId(id),
    onSuccess: (_, variables) => {
      message.success(
        variables.action === "approve"
          ? "Booking approved. Payment is now pending."
          : "Booking rejected.",
      );
  ...
  const columns = [
    {
      title: "Room",
      render: (_, record) =>
        record.room?.roomNumber ? `#${record.room.roomNumber}` : "—",
    },
    {
      title: "Guest",
    ...
    <div className="space-y-4">
      <div>
        ...
        <Text className="text-gray-400 text-sm">
          Review and approve pending room bookings before payment.
        </Text>
      </div>
      {isError && (
        <Alert
          ...
          showIcon
          message={
            error?.response?.data?.message || "Unable to load pending bookings."
          }
        />
      )}
      <Card>
